            ...
            <div className="px-4 sm:px-6 animate-in slide-in-from-bottom-2 duration-500 delay-100">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {activeDeals.map((deal) => {
                        ...
                        const isMatchedForUser = deal.timeLeft === "MATCHED FOR YOU";
                        return (
                            <div key={deal.id} className="w-full bg-white dark:bg-navy-900 rounded-2xl p-3 shadow-sm border border-slate-100 dark:border-navy-800 relative group transition-all duration-300 hover:shadow-lg flex flex-col justify-between">
                                    ...
                                    <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
                                        <div>
                                            <div className="flex justify-between items-start mb-1 gap-1">
                                                ...
                                                <span className={`text-[9px] font-bold px-1.5 py-0.5 rounded shrink-0 whitespace-nowrap ${isMatchedForUser ? 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400' : 'text-red-500 bg-red-50 dark:bg-red-900/20'}`}>
                                                    {deal.timeLeft}
                                                </span>
                                            </div>
                                ...
                                <div className="mt-3 flex justify-between items-center text-xs pt-2 border-t border-slate-50 dark:border-navy-800">
                                    ...
                                    <button
                                        ...
                                        disabled={isGrabbed}
                                        className={`px-3 py-1.5 rounded-lg font-bold transition-all text-xs ${isGrabbed ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : isMatchedForUser ? 'bg-gradient-to-r from-amber-500 to-rose-500 text-white animate-pulse shadow-lg shadow-rose-500/30 hover:scale-105' : 'bg-navy-900 dark:bg-white text-white dark:text-navy-900 hover:scale-105 active:scale-95'}`}
                                    >
                                        {isGrabbed ? (
                                            <span className="flex items-center"><Check size={12} className="mr-1" /> Claimed</span>
                                        ) : isMatchedForUser ? (
                                            <span className="flex items-center"><Zap size={12} className="mr-1" /> Claim Match</span>
                                        ) : (
                ...
                {user.activeWatches && user.activeWatches.length > 0 && (
                    <div className="mt-8 mb-4">
                        <div className="flex items-center gap-2 mb-3">
                            <Clock size={18} className="text-slate-400" />
                            <h4 className="font-bold text-sm text-navy-900 dark:text-white">Your Active Watches</h4>
                            <span className="bg-slate-100 dark:bg-navy-800 text-slate-500 dark:text-slate-400 px-2 py-0.5 rounded-full text-[10px] font-bold">{user.activeWatches.filter(w => w.status === 'Active').length} Active</span>
                        </div>

                        <div className="space-y-3">
                            {user.activeWatches.map((watch) => (
                                <div key={watch.id} className={`flex items-center justify-between p-3 rounded-xl border ${watch.status === 'Matched' ? 'bg-amber-50 border-amber-200 dark:bg-amber-900/10 dark:border-amber-900/30' : 'bg-white dark:bg-navy-900 border-slate-100 dark:border-navy-800'}`}>
                                    <div className="flex items-center gap-3">
                                        <div className={`w-8 h-8 rounded-full flex items-center justify-center ${watch.status === 'Matched' ? 'bg-amber-100 text-amber-600 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-slate-100 text-slate-400 dark:bg-navy-800'}`}>
                                            {watch.status === 'Matched' ? <Check size={16} /> : <Clock size={16} />}
                                        </div>
                                        <div className="min-w-0">
                                            <h5 className="font-bold text-xs text-navy-900 dark:text-white truncate">{watch.resortName}</h5>
                                            <p className="text-[10px] text-slate-500 dark:text-slate-400">
                                                {formatDate(watch.checkIn)} - {formatDate(watch.checkOut)}
                                            </p>
                                        </div>
                                    </div>
                                    <div>
                                        <span className={`text-[10px] font-bold px-2 py-1 rounded-full ${watch.status === 'Matched' ? 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400' : 'bg-slate-50 text-slate-500 dark:bg-navy-800 dark:text-slate-400'}`}>
                                            {watch.status === 'Matched' ? 'MATCHED' : 'Awaiting Cancellation'}
                                        </span>
                                    </div>
                                </div>
                            ))}
